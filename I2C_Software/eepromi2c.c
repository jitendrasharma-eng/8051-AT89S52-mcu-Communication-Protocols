...
void I2C_DataWrite(unsigned char Data) {
    ...
    // Acknowledge bit (read mode)
    SDA = 1;  // Release SDA for ACK bit
    SCL = 1;
    delay(5);
    ack = SDA;  // Read ACK
    SCL = 0;
   ...
   // return ack;  // Return 0 if ACK received, 1 if NACK
}
*************/
//Read byte of data from slave to master
